...
def handle_upload(file_path):
    ...
    for _, row in df.iterrows():
        try:
            order_num = "'" + str(row[2])
            buyer = row[24]
            customer = row[26].replace("님", "")
            product = row[10]
            option = row[11]
            amount = int(row[22])
            b_tel = row[27]
            address = row[29].strip('"')
            message = row[30]
            ...
            if buyer == customer:
                ...
                buyer = "고객사(" + buyer + ")"
            data = [order_num, buyer, customer + "님", b_tel, '', '', address, product, option, amount, message]
            sheet.append(data)

        except Exception as e:
            print(f"데이터 처리 중 오류 발생: {e}")

    base_excel_file_name = nowDate.strftime("%Y%m%d") + "_쿠팡_송장양식"
    excel_file_ext = ".xlsx"
